    ...
    var options = {
        ...
        tmpl_path: './views/calendar/tmpls/', 
        tmpl_cache: false,
    ...
    function add(){
        $.post("./views/calendar3/new.php?").done(function(data){$('#subContent').html(data)});
    }
    ...
    function cancel(id){
        $.confirm({
            title: '&#161;Advertencia!',
            type: 'orange',
            content: 'Realmente desea cancelar la cita?<br>Este proceso es irreversible.',
            buttons: {
                SI: function () {
                    $.ajax({
                        type: "POST",
